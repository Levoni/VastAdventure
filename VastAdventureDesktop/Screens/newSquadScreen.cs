...
namespace VastAdventureDesktop.Screens
{
   ...
   public class newSquadScreen : BasicScreen
   {
      ...
      public override void Init()
      {
         ...
         bar = new ProgressBar("bar", currentCharacter.experience + "/" + (currentCharacter.level * 100).ToString(), new EngineRectangle(viewport.X + viewport.Width / 2, viewport.Y + viewport.Height / 30 * 6 + 100, viewport.Width / 2 - 50, 50), Color.White, "dark_gray", Color.White);
         ...
         //Stat section
         lblMaxHealth = new Label("lblMaxHealth", "Max Health: " + currentCharacter.maxHealth.ToString(), new EngineRectangle(viewport.X + 50, viewport.Y + 50 + viewport.Height / 30 * 9, (viewport.Width) / 2 - 100, (viewport.Height / 30 * 2)), Color.DarkGray);
         lblMaxHealth.minFontScale = 0;

         lblStrength = new Label("lblStrength", "Strength: " + currentCharacter.strength.ToString(), new EngineRectangle(viewport.X + 50, viewport.Y + 50 + viewport.Height / 30 * 11, (viewport.Width) / 2 - 100, (viewport.Height / 30 * 2)), Color.DarkGray);
         lblStrength.minFontScale = 0;

         lblToughness = new Label("lblToughness", "Toughness: " + currentCharacter.toughness.ToString(), new EngineRectangle(viewport.X + viewport.Width / 2 + 50, viewport.Y + 50 + viewport.Height / 30 * 9, (viewport.Width) / 2 - 100, (viewport.Height / 30 * 2)), Color.DarkGray);
         lblToughness.minFontScale = 0;

         lblDexterity = new Label("lblDexterity", "Dexterity: " + currentCharacter.dexterity.ToString(), new EngineRectangle(viewport.X + viewport.Width / 2 + 50, viewport.Y + 50 + viewport.Height / 30 * 11, (viewport.Width) / 2 - 100, (viewport.Height / 30 * 2)), Color.DarkGray);
