...
namespace VastAdventureAndriod.Screens
{
   ...
   public class LocationScreen: BasicScreen
   {
      ...
      public void Init(int eventId)
      {
         ...
         //for (int i = 0; i < locationScreenModel.connectedLocations.Count; i++)
         //{
         //   bool precheckPassed = true;
         //   foreach(Action a in locationScreenModel.connectedLocations[i].PreChecks)
         //   {
         //      precheckPassed = PerformCheck(a);
         //   }
         //   if (precheckPassed)
         //   {
         //      Button tempButton = new Button("button" + i, "Go To " + locationScreenModel.connectedLocations[i].LocationId, new EngineRectangle(viewport.X + 50, (viewport.Height / 10 * (9 - i - 1)) - 50, viewport.Width - 100, viewport.Height / 10), Color.White);
         //      ButtonActionLink.Add(tempButton.Name, locationScreenModel.connectedLocations[i].Actions);
         //      tempButton.onClick = new EHandler<OnClick>(new Action<object, OnClick>(HandleClick));
         //      buttons.Add(tempButton);
         //      tempButton.setImageReferences("button_outline_none", "button_outline_hover", "button_outline_pressed", "button_outline_released");
         //      tempButton.padding = new int[] { 0, 30, 0, 30 };
         //      tempButton.minFontScale = 0;
         //      tempButton.maxFontScale = 5;
         //   }
         //}
      }
