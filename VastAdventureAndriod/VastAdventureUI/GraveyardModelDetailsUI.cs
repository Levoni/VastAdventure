...
namespace VastAdventureAndriod.VastAdventureUI
{
   ...
   public class GraveyardModelDetailsUI: Control
   {
      ...
      public void UpdateCharacter(GraveyardModel graveyardModel)
      {
         ...
         {
            Label newLabel = new Label("Item:" + i.ToString(), "- " + graveyardModel.inventory.items[i].itemDescription, new EngineRectangle(bounds.X + bounds.Width / 2, bounds.Y + bounds.Height / 9 * (1 * i + 2), bounds.Width / 2, bounds.Height / 9), Color.Black);
            newLabel.padding[3] = newLabel.padding[1] = 25;
            inventoryControls.Add(newLabel);
         }
      }
